...
mod tests {
    ...
    fn stale_temp_file_does_not_block_thumbnail_cache_recovery() {
        ...
        assert!(
            stale.is_file(),
            "unrelated temp files must not be deleted because another writer may own them"
        );

        let cached = load_cached(&cache_dir, &source)
            .expect("committed cache should reload after stale temp");
        assert!(cached.width() <= CACHE_EDGE);
        assert!(cached.height() <= CACHE_EDGE);
        let _ = std::fs::remove_dir_all(dir);
    }
