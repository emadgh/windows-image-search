...
impl ImageSearchApp {
    ...
    fn choose_similarity_image(&mut self) {
        ...
        indexer::spawn_similarity_search(
            self.db_path.clone(), self.model_cache.clone(), path, self.tx.clone(),
        );
    }
    ...
    pub(super) fn visible_indices(&self) -> Vec<usize> {
        let tokens: Vec<String> = self.search_text.split_whitespace()
            .map(str::to_ascii_lowercase).collect();
        self.source().iter().enumerate().filter(|(_, record)| {
            if !tokens.is_empty() {
                let haystack = format!("{} {} {} {}", record.file_name,
                    record.path.display(), record.description, record.keywords).to_ascii_lowercase();
                if !tokens.iter().all(|token| haystack.contains(token)) { return false; }
            }
            !self.color_enabled || views::color_distance(record.dominant, self.target_color) <= self.color_tolerance
        }).map(|(index, _)| index).collect()
    }

    pub(super) fn thumbnail(&mut self, ctx: &egui::Context, path: &Path) -> Option<TextureHandle> {
        if let Some(texture) = self.textures.get(path) { return Some(texture.clone()); }
        ...
        let texture = ctx.load_texture(path.to_string_lossy(),
            ColorImage::from_rgba_unmultiplied(size, &pixels), TextureOptions::LINEAR);
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                if ui.button("＋ Add folder").clicked() { self.add_folder(); }
                if ui.add_enabled(!self.busy && !self.roots.is_empty(), egui::Button::new("⟳ Rescan")).clicked() { self.start_rescan(); }
                if ui.add_enabled(!self.busy && !self.images.is_empty(), egui::Button::new("◉ Search by image")).clicked() { self.choose_similarity_image(); }
                if self.similarity_results.is_some() && ui.button("Clear image search").clicked() {
                    self.similarity_results = None; self.query_image = None;
                }
                ui.separator();
                ui.label("Search:");
                ui.add(egui::TextEdit::singleline(&mut self.search_text)
                    .hint_text("filename, path, description, keywords…").desired_width(320.0));
                ui.checkbox(&mut self.color_enabled, "Color");
                if self.color_enabled {
                    ui.color_edit_button_srgb(&mut self.target_color);
                    ui.add(egui::Slider::new(&mut self.color_tolerance, 0.03..=0.70).text("Tolerance"));
                }
            });
            ui.horizontal(|ui| {
        ...
        egui::SidePanel::left("folders").resizable(true).default_width(245.0).show(ctx, |ui| {
            ...
            for root in &self.roots {
                ui.horizontal(|ui| {
                    ...
                    if ui.small_button("×").clicked() { remove = Some(root.clone()); }
                });
            }
            if let Some(root) = remove { self.remove_folder(&root); }
            if self.roots.is_empty() { ui.label("Add a folder, then run Rescan."); }
            if let Some(query) = &self.query_image {
                ui.separator(); ui.strong("Similarity query");
                ui.label(views::truncate_middle(&query.display().to_string(), 32));
            }
            if let Some(error) = &self.last_error {
                ui.separator(); ui.colored_label(egui::Color32::LIGHT_RED, error);
            }
        });
            ...
            ui.horizontal(|ui| {
                ...
                if let Some((done, total)) = self.progress.filter(|(_, total)| *total > 0) {
                    ui.add(egui::ProgressBar::new(done as f32 / total as f32)
                        .desired_width(180.0).text(format!("{done}/{total}")));
                }
            });
        });
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.strong(format!("{} result{}", visible.len(), if visible.len() == 1 { "" } else { "s" }));
                if self.similarity_results.is_some() { ui.small("CLIP similarity order"); }
            });
            ui.separator();
            if visible.is_empty() {
                ui.centered_and_justified(|ui| { ui.label(if self.images.is_empty() {
                    "No indexed images yet." } else { "No images match the current filters." }); });
