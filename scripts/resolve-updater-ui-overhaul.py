...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise SystemExit(f"missing merge target: {label}")
    return text.replace(old, new, 1)


# Resolve src/ui/mod.rs by keeping the UI overhaul as the base and injecting
# updater state/calls in the corresponding modern locations.
...
# Resolve Preferences by preserving the overhaul's Appearance category and
# adding Updates as a sixth category. Collections remain a first-class workspace.
...
text = replace_once(
    ...
    "    Performance,\n    Storage,\n    Updates,\n}\n\nimpl SettingsCategory {\n    const ALL: [Self; 6] = [\n        Self::Appearance,\n        Self::SearchClip,\n        Self::FacesPeople,\n        Self::Performance,\n        Self::Storage,\n        Self::Updates,\n    ];\n",
    "Updates category and category list",
)
text = replace_once(
    text,
...
text = replace_once(
    ...
    "Updates category dispatch",
)

updates_fn = r'''fn settings_updates(app: &mut ImageSearchApp, ui: &mut egui::Ui, effects: &mut Effects) {
    section_title(
        ui,
        "Updates",
...
text = replace_once(
    ...
    updates_fn + "fn settings_storage(app: &mut ImageSearchApp, ui: &mut egui::Ui, effects: &mut Effects) {\n",
    "settings_updates function",
)

update_effects = r'''    if effects.save_update_settings {
...
text = replace_once(
    ...
    update_effects + "    if effects.save_face_settings {\n",
    "updater apply effects",
)
